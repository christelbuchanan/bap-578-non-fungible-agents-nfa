...
3. **Hybrid On-Chain/Off-Chain Architecture**: Clear separation between on-chain identity and permissions versus off-chain extended imprint and complex behaviors, optimizing for both gas efficiency and rich functionality.
...
| Extended Imprint        | Off-chain (with hash verification)   | Rich imprint would be prohibitively expensive on-chain     |
...
3. **Cross-Platform Compatibility**: Agents can move between applications while maintaining their identity, imprint, and learned behaviors.
...
- **Enhanced Metadata**: Rich metadata structure with persona, imprint, voice, and animation capabilities
- **Imprint Modules**: Support for external imprint sources with cryptographic verification
...
- **`ImprintModuleRegistry.sol`**: Registry for managing external imprint modules with cryptographic verification
...
- **imprint**: Short summary string describing the agent's default role or purpose
...
### Traditional Agents (JSON Light Imprint)
...
### Working with Imprint Modules

To register a imprint module:
...
- **Cryptographic Verification**: Signature-based verification for imprint module registration and vault access
