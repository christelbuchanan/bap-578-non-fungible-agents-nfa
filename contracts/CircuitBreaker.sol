...
contract CircuitBreaker is Initializable, OwnableUpgradeable {
     ...
     * @param paused The new pause state
     */
    function setContractPause(address contractAddress, bool paused) 
        external 
        onlyAuthorized 
    {
     ...
     */
    function isContractPaused(address contractAddress) 
        external 
        view 
        returns (bool) 
    {
     ...
     */
    function setEmergencyMultiSig(address _emergencyMultiSig) 
        external 
        onlyAuthorized 
    {
        require(_emergencyMultiSig != address(0), "CircuitBreaker: multi-sig is zero address");
