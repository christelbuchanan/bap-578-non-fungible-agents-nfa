...
contract VaultPermissionManager is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function hasVaultAccess(uint256 tokenId, address delegate) 
        external 
        view 
        returns (bool) 
    {
        ...
        return _delegatedAccess[tokenId][delegate] && _delegationExpiry[tokenId][delegate] > block.timestamp;
    }
     ...
     */
    function getDelegationExpiry(uint256 tokenId, address delegate) 
        external 
        view 
        returns (uint256) 
    {
