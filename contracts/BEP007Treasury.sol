...
contract BEP007Treasury is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        require(_foundationWallet != address(0), "BEP007Treasury: foundation wallet is zero address");
     ...
     */
    function collectProtocolFee(address agentMinter) 
        external 
        payable 
        onlyAgentFactory 
        whenFeeCollectionNotPaused 
        nonReentrant 
    {
    ...
    function setFoundationWallet(address newFoundationWallet) external onlyGovernance {
        require(newFoundationWallet != address(0), "BEP007Treasury: foundation wallet is zero address");
    ...
    function setStakingPoolWallet(address newStakingPoolWallet) external onlyGovernance {
        require(newStakingPoolWallet != address(0), "BEP007Treasury: staking wallet is zero address");
     ...
     */
    function getEntityWallets() external view returns (
        address foundation,
        address treasury,
        address staking
    ) {
     ...
     */
    function calculateDistribution(uint256 feeAmount) external pure returns (
        uint256 foundationAmount,
        uint256 treasuryAmount,
        uint256 stakingAmount
    ) {
