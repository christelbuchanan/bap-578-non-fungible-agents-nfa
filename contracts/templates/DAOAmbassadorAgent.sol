...
contract DAOAmbassadorAgent is Ownable, ReentrancyGuard {
    ...
    event CommunicationEffectivenessRecorded(uint256 indexed communicationId, uint256 effectivenessScore);
    ...
    modifier whenLearningEnabled() {
        require(learningEnabled && learningModule != address(0), "DAOAmbassadorAgent: learning not enabled");
        _;
    }
    ...
    function enableLearning(address _learningModule) external onlyOwner {
        require(_learningModule != address(0), "DAOAmbassadorAgent: learning module is zero address");
        ...
        onlyOwner 
    {
        ...
        require(_communityEngagementGoal <= 100, "DAOAmbassadorAgent: engagement goal must be 0-100");
        ...
        if (learningEnabled) {
            this.recordInteraction("profile_update", true, abi.encode(_autonomyLevel, _communityEngagementGoal));
        }
    }
        ...
        returns (uint256 communicationId) 
    {
        ...
        // Record learning interaction
        if (learningEnabled) {
            this.recordInteraction("communication_draft", true, abi.encode(_topics, _reachEstimate));
        }
        ...
        returns (uint256 communicationId) 
    {
        ...
        emit CommunicationCreated(communicationId, _communicationType, _platform);
        emit AutonomousActionTaken("autonomous_communication", abi.encode(communicationId), block.timestamp);
    ...
    function recordCommunicationEffectiveness(
        ...
        uint256 sentimentScore = totalReactions > 0 ? (_positiveReactions * 100) / totalReactions : 50;
        ...
        // Record learning interaction
        if (learningEnabled) {
            this.recordInteraction("effectiveness_analysis", true, abi.encode(effectivenessScore, sentimentScore));
        }
    }
    ...
    function updateCommunityInsights(
        ...
        // Record learning interaction
        if (learningEnabled) {
            this.recordInteraction("insights_update", true, abi.encode(_optimalProposalDuration, _averageDecisionTime));
        }
    }
        ...
        returns (uint256 proposalId) 
    {
        ...
        // Record learning interaction
        if (learningEnabled) {
            this.recordInteraction("proposal_creation", true, abi.encode(_useOptimalTiming, duration));
        }
    ...
    function _estimateReach(
        ...
        for (uint256 i = 0; i < topics.length; i++) {
            for (uint256 j = 0; j < communityInsights.trendingTopics.length; j++) {
                if (keccak256(bytes(topics[i])) == keccak256(bytes(communityInsights.trendingTopics[j]))) {
    ...
    function getCommunityInsights() 
        external 
        view 
        returns (CommunityInsights memory) 
    {
     ...
     */
    function getCommunicationEffectiveness(uint256 _communicationId) 
        external 
        view 
        returns (CommunicationEffectiveness memory) 
    {
        require(_communicationId <= communicationCount && _communicationId > 0, "DAOAmbassadorAgent: communication does not exist");
        ...
        onlyDAOContract 
    {
        ...
        // Record learning interaction
        if (learningEnabled) {
            this.recordInteraction("proposal_execution", true, abi.encode(_participationRate, _consensusScore));
        }
    }
        ...
        onlyDAOContract 
    {
        ...
        if (previousMembers > 0) {
            if (_activeMembers > previousMembers) {
                metrics.communityGrowthRate = ((_activeMembers - previousMembers) * 100) / previousMembers;
            } else {
        ...
        // Record learning interaction
        if (learningEnabled) {
            this.recordInteraction("member_update", true, abi.encode(_activeMembers, metrics.communityGrowthRate));
        }
    }
}
