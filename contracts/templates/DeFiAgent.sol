...
contract DeFiAgent is Ownable {
     ...
     */
    function analyzeMarket(address tokenAddress) 
        external 
        onlyOwner 
        returns (MarketAnalysis memory analysis) 
    {
        require(allowedTokens[tokenAddress] || tokenAddress == address(0), "DeFiAgent: token not allowed");
        ...
        onlyAgentToken 
    {
        ...
        if (learningEnabled) {
            ...
            require(confidence >= strategy.confidenceThreshold, "DeFiAgent: confidence below threshold");
            
            // Additional learning-based validation
            require(_validateLearningBasedTrade(tokenIn, tokenOut, analysis), "DeFiAgent: learning validation failed");
        } else {
    ...
    function getLearningMetrics() 
        external 
        view 
        returns (DeFiLearningMetrics memory) 
    {
     ...
     */
    function getMarketAnalysisHistory(uint256 count) 
        external 
        view 
        returns (MarketAnalysis[] memory) 
    {
     ...
     */
    function getTradeHistory(uint256 count) 
        external 
        view 
     ...
     */
    function _updateMarketIndicators(uint256 currentPrice) 
        internal 
    {
        // Update volatility index (simplified calculation)
        uint256 priceChange = currentPrice > movingAverage ? 
            currentPrice - movingAverage : movingAverage - currentPrice;
     ...
     */
    function _calculateTrendStrength() 
        internal 
        view 
        returns (uint256) 
    {
     ...
     */
    function _calculateRiskScore(uint256 volatility, uint256 trendStrength) 
        internal 
        pure 
        returns (uint256) 
    {
     ...
     */
    function _calculateOpportunityScore(uint256 volatility, uint256 trendStrength) 
        internal 
        pure 
        returns (uint256) 
    {
     ...
     */
    function _calculateAnalysisConfidence(uint256 volatility, uint256 trendStrength) 
        internal 
        pure 
        returns (uint256) 
    {
     ...
     */
    function _generateMarketSignal(uint256 trendStrength, uint256 opportunityScore) 
        internal 
        pure 
        returns (bool) 
    {
     ...
     */
    function _updateProfitabilityScore() 
        internal 
    {
        ...
        uint256 successRate = (learningMetrics.successfulTrades * 100) / learningMetrics.totalTrades;
        
        // Calculate average P&L (simplified)
        uint256 totalPnL = 0;
        uint256 validTrades = 0;
        
        for (uint256 i = 1; i <= tradeCount && i <= 10; i++) { // Last 10 trades
