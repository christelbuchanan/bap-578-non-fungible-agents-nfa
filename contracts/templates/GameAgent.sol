...
contract GameAgent is Ownable {
    ...
    function analyzePlayerBehavior(
        ...
        external 
        onlyGameContract 
        returns (PlayerBehavior memory behavior) 
    {
        ...
        // Update win rate
        if (outcome) {
            behavior.winRate = ((behavior.winRate * (behavior.totalInteractions - 1)) + 0) / behavior.totalInteractions;
        } else {
            behavior.winRate = ((behavior.winRate * (behavior.totalInteractions - 1)) + 100) / behavior.totalInteractions;
        }
    ...
    function makeStrategicDecision(
        ...
        external 
        onlyGameContract 
        returns (uint256 chosenAction, uint256 confidence) 
    {
        require(availableActions.length > 0, "GameAgent: no available actions");
        
        if (learningEnabled && strategy.playerSpecificStrategy && playerBehaviors[player].player != address(0)) {
     ...
     */
    function adaptStrategy(string calldata reason) 
        external 
        onlyOwner 
    {
        ...
        onlyGameContract 
    {
        ...
        if (attributes.experience >= experienceRequired) {
            ...
            uint256 learningBonus = learningEnabled ? 
                (attributes.adaptability * baseGain) / 100 : 0;
     ...
     */
    function addQuest(uint256 questId, string calldata questType) 
        external 
        onlyGameContract 
    {
        ...
        onlyGameContract 
    {
        ...
        if (progress >= 100) {
            ...
            uint256 learningBonus = learningEnabled ? 
                (attributes.strategicThinking * baseExperience) / 1000 : 0;
    ...
    function getContextualDialogue(
        ...
        external 
        view 
        returns (uint256 dialogueId, string memory dialogue, uint256 confidence) 
    {
        if (learningEnabled && strategy.playerSpecificStrategy && playerBehaviors[player].player != address(0)) {
    ...
    function getLearningMetrics() 
        external 
        view 
        returns (GameLearningMetrics memory) 
    {
     ...
     */
    function getPlayerBehavior(address player) 
        external 
        view 
        returns (PlayerBehavior memory) 
    {
     ...
     */
    function getInteractionHistory(uint256 count) 
        external 
        view 
        returns (InteractionRecord[] memory) 
    {
    ...
    function _analyzeTradeBehavior(
        ...
        if (playerAction >= 60) { // Fair trade
            behavior.cooperationLevel = (behavior.cooperationLevel * 9 + 80) / 10;
        } else if (playerAction <= 30) { // Unfair trade
    ...
    function _analyzeDialogueBehavior(
        ...
        if (playerAction >= 70) { // Friendly response
            behavior.cooperationLevel = (behavior.cooperationLevel * 9 + 75) / 10;
        } else if (playerAction <= 30) { // Hostile response
    ...
    function _analyzeQuestBehavior(
        ...
        if (playerAction >= 80) { // Aggressive quest approach
            behavior.aggressionLevel = (behavior.aggressionLevel * 9 + 70) / 10;
            behavior.riskTolerance = (behavior.riskTolerance * 9 + 70) / 10;
        } else if (playerAction <= 40) { // Cautious approach
    ...
    function _updatePredictability(
        ...
        if (behavior.totalInteractions > 5) {
            ...
            uint256 deviation = playerAction > expectedAction ? 
                playerAction - expectedAction : expectedAction - playerAction;
            
            if (deviation <= 10) {
                behavior.predictability = behavior.predictability < 95 ? 
                    behavior.predictability + 1 : 100;
            } else if (deviation >= 30) {
                behavior.predictability = behavior.predictability > 5 ? 
                    behavior.predictability - 1 : 0;
            }
        }
    }
     ...
     */
    function _chooseDefaultAction(uint256[] calldata availableActions) 
        internal 
        pure 
        returns (uint256 chosenAction, uint256 confidence) 
    {
     ...
     */
    function _predictItemEffectiveness(uint256 itemId) 
        internal 
        view 
        returns (uint256 effectiveness) 
    {
     ...
     */
    function _predictQuestOutcome(string calldata questType) 
        internal 
        view 
        returns (uint256 difficulty, uint256 successProbability) 
    {
        // Base predictions
        difficulty = 50;
        ...
        uint256 agentCapability = (attributes.strength + attributes.intelligence + 
                                  attributes.agility + attributes.strategicThinking) / 4;
     ...
     */
    function _updateStrategyEffectiveness() 
        internal 
    {
        ...
        uint256 successRate = (learningMetrics.successfulInteractions * 100) / learningMetrics.totalInteractions;
     ...
     */
    function _updatePlayerPredictionAccuracy() 
        internal 
    {
        ...
        if (totalPredictions > 0) {
            learningMetrics.playerPredictionAccuracy = (correctPredictions * 100) / totalPredictions;
        }
    }
}
