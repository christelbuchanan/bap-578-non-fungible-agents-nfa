...
contract AgentFactory is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event AgentLearningEnabled(address indexed agent, uint256 indexed tokenId, address learningModule);
    ...
    {
        require(approvedTemplates[params.logicAddress], "AgentFactory: logic template not approved");
        ...
        IBEP007.AgentMetadata memory enhancedMetadata = params.extendedMetadata;
        if (params.enableLearning) {
            enhancedMetadata.learningEnabled = true;
            enhancedMetadata.learningModule = params.learningModule;
            enhancedMetadata.learningTreeRoot = params.initialLearningRoot;
            enhancedMetadata.learningVersion = 1;
        }
     ...
     * @param module The address of the learning module contract
     */
    function revokeLearningModule(address module) 
        external 
        onlyGovernance 
    {
     ...
     */
    function updateLearningConfig(LearningConfig memory config) 
        external 
        onlyGovernance 
    {
     ...
     */
    function setDefaultLearningModule(address newDefaultModule) 
        external 
        onlyGovernance 
    {
     ...
     */
    function getLatestLearningModule(string memory category) 
        external 
        view 
        returns (address) 
    {
    ...
    function getGlobalLearningStats() 
        external 
        view 
        returns (LearningGlobalStats memory) 
    {
     ...
     */
    function isLearningModuleApproved(address module) 
        external 
        view 
        returns (bool) 
    {
     ...
     */
    function setLearningPaused(bool paused) 
        external 
        onlyGovernance 
    {
