...
async function upgradToLearning(bep007Enhanced, merkleTreeLearning, tokenId, owner) {
  ...
  const tx = await bep007Enhanced.enableLearning(tokenId, merkleTreeLearning.address, initialRoot);
...
async function recordLearningInteraction(bep007Enhanced, tokenId, interactionType, success) {
  ...
  const tx = await bep007Enhanced.recordInteraction(tokenId, interactionType, success);
...
async function verifyLearningClaim(bep007Enhanced, tokenId, claim, proof) {
  ...
  const isValid = await bep007Enhanced.verifyLearningClaim(tokenId, claim, proof);
